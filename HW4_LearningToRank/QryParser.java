...
public class QryParser {
    ...
    private static Qry createOperator(String operatorName) {
        ...
        if (operatorNameLowerCase.startsWith("#near")) {
            operatorDistance = Integer.valueOf(operatorNameLowerCase.substring(6));
            operatorNameLowerCase = operatorNameLowerCase.substring(0, 5);
        }

        if (operatorNameLowerCase.startsWith("#window")) {
            operatorDistance = Integer.valueOf(operatorNameLowerCase.substring(8));
            operatorNameLowerCase = operatorNameLowerCase.substring(0, 7);
        }
    ...
    private static Qry parseString(String queryString)
        ...
        while (queryString.length() > 0) {
            ...
            if (queryTree instanceof QrySopWAnd) {
                ...
                ((QrySopWAnd) queryTree).ttlWeights = ((QrySopWAnd) queryTree).sumWeights(weights);
            }
            if (queryTree instanceof QrySopWSum) {
                ((QrySopWSum) queryTree).weights = weights;
                ((QrySopWSum) queryTree).ttlWeights = ((QrySopWSum) queryTree).sumWeights(weights);
            }
